...
本项目的机器人模型单臂 7 自由度，这些关节进入策略。头、腰、腿的关节名和数量以随后入库的 URDF/MJCF 为准。模型文件由本地工程放入仓库，并用 git 管理；在此之前不训练策略。

篮球约 0.62 kg、直径约 24.6 cm。手指不进入策略动作空间。v1 手部只有一种锁死的张开托球手型。腕部属于 7 自由度手臂，可以参与推球、传球和投篮。
...
- 传球用双手前推。
- 投篮用推射，单臂 7 自由度里的腕部关节参与动作。后旋和拨指不在 v1。
- 掌心加软垫。接球时降低手臂阻抗，避免刚性手指把球弹飞。
- 运球冲击是否顶满关节力矩，等模型文件入库后在仿真里检查。
...
仿真环境、拣球奖励和实机视觉等模型文件入库后再做。单臂按 7 自由度读入，关节表与 `dof_names` 以该模型为准。
